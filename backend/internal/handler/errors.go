...
func handleError(c *gin.Context, err error) {
	...
	case errors.Is(err, service.ErrCorrectionConflict):
		util.Fail(c, http.StatusConflict, "correction_conflict", err.Error())
	case errors.Is(err, service.ErrCorrectionNotOpen):
		util.Fail(c, http.StatusConflict, "correction_not_open", err.Error())
	...
	case errors.Is(err, service.ErrInvalidTransition), errors.Is(err, service.ErrInvalidInput),
		...
		errors.Is(err, service.ErrSuperseded):
